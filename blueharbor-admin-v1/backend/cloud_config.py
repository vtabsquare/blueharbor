...
load()
URL=os.environ.get('SUPABASE_URL','https://atlubtjwhsridpqeutao.supabase.co').rstrip('/')
...
def validate():
    ...
    missing=[k for k in ('SUPABASE_PUBLISHABLE_KEY','SUPABASE_SECRET_KEY','SUPABASE_DB_URL') if not os.environ.get(k) or any(p in os.environ[k].upper() for p in placeholders)]
    if missing:raise RuntimeError('Complete backend/.env settings for this application: '+', '.join(missing))
    parsed=urlparse(URL)
    if parsed.scheme!='https' or not parsed.hostname or not parsed.hostname.endswith('.supabase.co'):raise RuntimeError('Use your HTTPS Supabase project URL.')
