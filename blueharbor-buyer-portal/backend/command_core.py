...
def monitoring(c):
    ...
    model=cfg.get('ai_model') or os.environ.get('BLUEHARBOR_AI_MODEL','')
    job=c.execute("SELECT status,updated,error FROM ai_jobs ORDER BY id DESC LIMIT 1").fetchone()
    running=c.execute("SELECT 1 FROM ai_jobs WHERE status='RUNNING' LIMIT 1").fetchone()
    state='NOT_CONFIGURED' if not model else 'OFFLINE' if not fresh else 'RUNNING' if running else 'FAILED' if job and job['status']=='FAILED' else 'SCHEDULED' if cfg.get('ai_auto') else 'MANUAL'
    return {'rules_status':'FAILED' if fresh and cfg.get('monitor_error') else 'RUNNING' if fresh else 'OFFLINE','model_status':state,'model':model,'last_scan':cfg.get('last_monitor'),'next_scan':cfg.get('next_monitor') if fresh else None,'error':cfg.get('monitor_error',''),'auto_analysis':bool(cfg.get('ai_auto')),'last_model_result':job['updated'] if job else None}
