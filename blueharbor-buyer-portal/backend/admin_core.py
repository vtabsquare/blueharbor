...
def case_revision(c,uid):
    ...
    c.execute("UPDATE ai_jobs SET status='STALE',error='Case data or document review changed; run a current analysis.',updated=? WHERE kind='VERIFICATION' AND entity_id=? AND status IN ('COMPLETED','QUEUED')",(S.now(),str(uid)))
...
def route(h,c,path,d,write):
    ...
    if endpoint.startswith('buyer/') and not write:
        ...
        return {'buyer':S.public_user(u),'case':dict(case) if case else {'revision':1},'documents':docs,'required':rules(c).get(u['country'],[]),'jobs':[dict(r) for r in c.execute("SELECT * FROM ai_jobs WHERE kind='VERIFICATION' AND entity_id=? ORDER BY id DESC LIMIT 20",(str(uid),))]}
    ...
    elif endpoint=='monitor-config':
        require(staff);model=text(d,'ai_model',150,False)
        if 'cloud' in model.lower():raise S.APIError('Select a local model, not a cloud-backed model.')
        values={'ai_model':model,'ai_auto':d.get('ai_auto') is True}
        for key,value in values.items():c.execute('INSERT OR REPLACE INTO settings VALUES(?,?)',(key,json.dumps(value)))
        record(c,staff,'MONITOR_CONFIGURED','monitor','Staff configured background model analysis',after=values)
    ...
    elif endpoint in ('ai-run','ai-retry','alert'): ai_action(c,staff,endpoint,d)
...
def snapshot(c,staff):
    ...
    return {'staff':public_staff(staff),'buyers':buyers,'orders':orders,'products':products,'lots':enrich_lots(c,lots),'tanks':tanks,
        ...
        'jobs':[dict(r) for r in c.execute('SELECT id,kind,entity_id,revision,status,attempts,model,output_json,error,created,updated FROM ai_jobs ORDER BY id DESC LIMIT 100') if (r['kind']=='VERIFICATION' and is_verify) or (r['kind']=='STOCK' and is_ops)],
        'documents':[dict(r) for r in c.execute('SELECT id,order_id,kind,name,mime,published,version,created FROM trade_documents ORDER BY created DESC')] if is_ops else [],
        ...
        'integrations':{'ai':bool(settings(c).get('ai_model') or os.environ.get('BLUEHARBOR_AI_MODEL')),'identity':'Not connected','email':'SMTP enabled' if configured() and settings(c).get('email_enabled') else 'Not connected','sensors':'Manual readings','carrier':'Manual updates'}}
...
def ai_action(c,staff,action,d):
    from admin_ai import enqueue
    if action=='ai-run':
        kind=text(d,'kind')
        if kind not in ('VERIFICATION','STOCK'):raise S.APIError('Invalid AI job type.')
        require(staff,'VERIFIER' if kind=='VERIFICATION' else 'OPERATIONS')
        entity=str(integer(d,'user_id',1)) if kind=='VERIFICATION' else 'inventory'
        enqueue(c,kind,entity);record(c,staff,'AI_REQUESTED',entity,'Staff requested analysis',after={'kind':kind})
    elif action=='ai-retry':
        job=c.execute('SELECT * FROM ai_jobs WHERE id=?',(integer(d,'id',1),)).fetchone()
        if not job:raise S.APIError('Job not found.',404)
        require(staff,'VERIFIER' if job['kind']=='VERIFICATION' else 'OPERATIONS')
        if job['status'] not in ('FAILED','STALE','BLOCKED'):raise S.APIError('Only failed, stale or blocked jobs can be retried.')
        enqueue(c,job['kind'],job['entity_id']);record(c,staff,'AI_RETRIED',job['id'],'New analysis requested')
    else:
...
def configuration(c,staff,action,d):
    ...
    if action=='country':
        ...
        entity='settings';values={'expiry_days':integer(d,'expiry_days',1,365),'monitor_seconds':integer(d,'monitor_seconds',15,3600),'handling_cents':integer(d,'handling_cents'),'ai_model':text(d,'ai_model',150,False),'ai_auto':d.get('ai_auto') is True,'email_enabled':d.get('email_enabled') is True}
        for key,value in values.items():c.execute('INSERT OR REPLACE INTO settings VALUES(?,?)',(key,json.dumps(value)))
...
def document_action(c,staff,action,d):
    ...
    c.execute('INSERT INTO trade_documents VALUES(?,?,?,?,?,?,?,?,?,?)',(did,oid,kind,name,mime,content,0,version,S.now(),staff['id']))
